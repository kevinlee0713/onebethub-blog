...
- 이 두 알림 다 **`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`가 GitHub Secrets에 등록돼야 실제로
  전송된다 — 2026-09-18 기준 아직 미등록**(`tg()`가 조용히 스킵). Kevin이 텔레그램 봇을 만들어서
  토큰과 채팅 ID를 전달해야 위 알림들이 실제로 동작한다.
