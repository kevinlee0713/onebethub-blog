...
14. **`isConversionPage()` 부분일치 버그 — `/seo검수`로 T2-00 draft를 검수하다 발견(2026-09-23)** —
    `(page.role || '').includes('전환')`가 `role="유입 허브(전환 아님)"`도 "전환"을 포함한다는 이유로
    전환 페이지로 오분류했다. 그 결과 `resolveCtaUrl()`이 같은 클러스터에 이미 발행된 실제 전환
    페이지(T1-01)로 내부 링크를 걸어야 할 유입 허브 글에서 곧바로 plain `https://7play.co/`(도어웨이
    패턴 폴백)를 반환했고, `ctaBlock()`도 전환페이지용 문구("7Play 솔루션 보기")를 그대로 썼다 —
    Claude 검증과 Gemini 교차검수 양쪽 모두 독립적으로 같은 문제를 지적했다. `role.includes('전환') &&
    !role.includes('아님')`로 수정(`generate-post.mjs:727`). 같은 검수에서 본문에 경쟁사 상업 링크
    2곳(나인솔루션 `xn--910br1nf9g2g92y.com`, 루미솔루션 `rumibetsolution.com`)이 출처처럼 섞여
    들어간 것도 발견 — 이건 파이프라인 프롬프트/게이트 쪽 문제라기보다 Serper 출처 검색이 우연히
    경쟁사 판매 페이지를 "실제 출처"로 집어온 사례로 보이며, 별도 자동 가드는 아직 없음(향후 반복되면
    출처 후보에서 알려진 경쟁사 도메인을 걸러내는 로직 검토 필요). 이미 생성된 draft(post 30 KO/32
    EN)는 코드 수정과 별개로 `wp_update_post`/`update_post_meta`로 직접 보정(CTA 내부링크 교체, 경쟁사
    링크 제거, KO 메타설명 96자→125자)했다 — 다음 재작성부터가 아니라 즉시 반영됨.
